// GeneralOverviewDashboardPage.tsx — clean, no red-lines
...
  gender: string; disability: string; school_type: string; school_level: string;
  class_grade: string; student_count: number; school_count: number;
  classroom_count: number; computer_count: number; infrastructure_score: number;
  key_entry_level?: string; is_o_level_student: number; school?: string;
};
type TeacherCapacityRow = {
  session: string; zone: string; state: string; lga: string; ward: string;
  school: string; gender: string; school_type: string; school_level: string;
  class_grade: string; qualification_group: string; qualification_status: string;
  disability?: string; student_count: number; teacher_count: number; pupil_teacher_ratio: number;
};
type TransitionDirectRow = {
  session: string; zone: string; state: string; lga: string; ward: string;
  school: string; gender: string; disability: string; exam_body: string;
  institution_type: string; ss3_total: number; o_level_candidates: number;
  utme_participants: number; admitted_students: number; matriculated_students: number;
  delayed_transition_students: number; median_time_to_matriculation_years: number;
};
type PolicyImpactRow = {
  session: string; zone: string; state: string; lga: string;
  institution_type: string; tertiary_institution: string; gender: string;
  disability: string; programme_cluster: string; discipline_group: string;
  programme: string; admitted_count: number; matriculated_count: number;
};
type PolicyLoanRow = {
  session: string; zone: string; state: string; lga: string;
  institution_type: string; tertiary_institution: string; gender: string;
  disability: string; programme_cluster: string; discipline_group: string;
  programme: string; loan_applications: number; loan_approved: number; loan_disbursed: number;
};

// ─── Shared structural types ──────────────────────────────────────────────────
type MapLevel = "state" | "lga";
type LocationLevel = "zone" | "state" | "lga" | "ward" | "school";
type DrillState = { zone?: string; state?: string; lga?: string; ward?: string };
type LegendItem = { label: string; color: string; dashed?: boolean; shape?: "bar" | "circle" };
type ChartBundle = {
  data: PlotlyData[];
...
  accent: string; bg: string; icon: ReactNode; help: string; showDelta?: boolean;
};
type BenchmarkMeta = { value: number; label: string; source: string };
type FacilityMetrics = {
  students: number; schools: number; classrooms: number; computers: number; infraScore: number;
};
type AggregatedGroup = {
  label: string; zone?: string; state?: string; lga?: string; metrics: FacilityMetrics;
};
type LossRow = { stage: string; from: number; to: number; lost: number; lostPct: number };
type AggregateMetrics = {
  ss3_total: number; o_level_candidates: number; utme_participants: number;
  admitted_students: number; matriculated_students: number;
  delayed_transition_students: number; median_time_to_matriculation_years: number;
};
type GroupedTrRow = { label: string; rows: TransitionDirectRow[]; metrics: AggregateMetrics };
type LocationChartResult = { bundle: ChartBundle; level: LocationLevel };
...
  title: string; explanation: string; mapData: SvgMapProps | null;
  drill: DrillState; onReset: () => void; onStateClick: (name: string) => void;
};
type ExpandKey = "density" | "ptr" | "dropoff" | "funnel" | "lossTable" | "stemm" | "loan" | "iqs";
type ExpandState = {
  key: ExpandKey;
  title: string;
  help: string;
...
// ─── Constants ────────────────────────────────────────────────────────────────
const COLORS = {
  text: "#0f172a", sub: "#64748b", grid: "rgba(15,23,42,0.10)", bg: "rgba(0,0,0,0)",
  tealStart: "#ccfbf1", tealEnd: "#115e59",
  ratio: "#0ea5e9", benchmark: "#ef4444",
  // Drop-off colors
  ss3: "#2563eb", olevel: "#10b981", utme: "#f59e0b", admit: "#8b5cf6",
  matric: "#14b8a6",   // changed from red → teal so Admitted→Matric is distinct
  // Tertiary trends
  stemm: "#2563eb", nonStemm: "#f59e0b",
  // Loan
  applications: "#2563eb", disbursed: "#f97316",
  // IQS chart
  iqs: "#ef4444", line: "#334155",
  // School level combo
  primary: "#2563eb", jss: "#10b981", sss: "#8b5cf6", vocational: "#f97316",
};
// UBE primary benchmark
const UBEC_BENCHMARK: BenchmarkMeta = {
  value: 35,
...
  totalTeachers: "Total teachers across all school types and levels within the current session and location filters.",
};
...
function normalizeGeoValue(value: string | undefined | null): string {
...
    .trim()
    .replace(/\s+/g, " ");
  if (["abuja federal capital territory", "federal capital territory", "abuja fct", "fct abuja"].includes(normalized)) return "abuja federal capital territory";
  if (normalized === "port harcourt") return "port harcourt";
  if (normalized === "ado ekiti") return "ado ekiti";
  return normalized;
}

function geoMatches(left: string | undefined | null, right: string | undefined | null): boolean {
  const rightNorm = normalizeGeoValue(right);
  if (!rightNorm) return true;
  return normalizeGeoValue(left) === rightNorm;
}
...
    const color = typeof raw === "string" ? raw : null;
...
function resolveTeacherLocationRows(
...
  if (filters.zone) {
    scopedRows = scopedRows.filter((row) => geoMatches(row.zone, filters.zone));
  }

  if (filters.state) {
    scopedRows = scopedRows.filter((row) => geoMatches(row.state, filters.state));
    currentLevel = "lga";
  }

  if (filters.lga) {
    scopedRows = scopedRows.filter((row) => geoMatches(row.lga, filters.lga));
    currentLevel = "ward";
  }

  if (filters.ward) {
    scopedRows = scopedRows.filter((row) => geoMatches(row.ward, filters.ward));
    currentLevel = "school";
  }
...
function teacherLocationLabel(row: TeacherCapacityRow, level: LocationLevel): string {
  if (level === "state") return row.state;
  if (level === "lga") return row.lga;
  if (level === "ward") return row.ward;
  return row.school;
...
function buildPupilTeacherRatioByLocationChart(
  rows: TeacherCapacityRow[],
  filters: MinisterFilters,
  benchmarkMeta: BenchmarkMeta,
): LocationChartResult | null {
  const primaryRows = rows.filter((row) => row.school_level === "Pre-Primary/Primary");
  const resolved = resolveTeacherLocationRows(primaryRows, filters, "state");
  const bucket = new Map<string, { students: number; teachers: number; publicStudents: number; publicTeachers: number; privateStudents: number; privateTeachers: number }>();

  resolved.rows.forEach((row) => {
    const label = teacherLocationLabel(row, resolved.level);
    if (!label) return;
...
      y: grouped.map((item) => shortLocationLabel(item.label)),
...
  layout.uirevision = `teacher-capacity-ptr-primary-${resolved.level}-${grouped.length}`;
...
        { label: `UBE Primary Benchmark ${fmtRatio(benchmarkMeta.value)}`, color: COLORS.benchmark, shape: "circle" },
      ],
    },
  };
}
...
function aggregateRows(rows: TransitionDirectRow[]): AggregateMetrics {
  return {
    ss3_total: rows.reduce((s, r) => s + safeNum(r.ss3_total), 0),
    o_level_candidates: rows.reduce((s, r) => s + safeNum(r.o_level_candidates), 0),
    utme_participants: rows.reduce((s, r) => s + safeNum(r.utme_participants), 0),
    admitted_students: rows.reduce((s, r) => s + safeNum(r.admitted_students), 0),
    matriculated_students: rows.reduce((s, r) => s + safeNum(r.matriculated_students), 0),
    delayed_transition_students: rows.reduce((s, r) => s + safeNum(r.delayed_transition_students), 0),
    median_time_to_matriculation_years: avg(rows.map(r => safeNum(r.median_time_to_matriculation_years))),
  };
}

function trLabel(row: TransitionDirectRow, level: LocationLevel): string {
  if (level === "zone") return row.zone;
  if (level === "state") return row.state;
  if (level === "lga") return row.lga;
  if (level === "ward") return row.ward ?? "";
...
function makeGroupedTr(rows: TransitionDirectRow[], level: LocationLevel): GroupedTrRow[] {
  const map = new Map<string, TransitionDirectRow[]>();
  rows.forEach(row => {
    const key = trLabel(row, level);
    if (!key) return;
    const cur = map.get(key); if (cur) cur.push(row); else map.set(key, [row]);
  });
...
function resolveTransitionRows(
  rows: TransitionDirectRow[], baseLevel: LocationLevel, drill: DrillState, filters: MinisterFilters,
): { level: LocationLevel; rows: TransitionDirectRow[] } {
  let r = rows; let level = baseLevel;
  const next = (cur: LocationLevel): LocationLevel =>
    cur === "zone" ? "state" : cur === "state" ? "lga" : cur === "lga" ? "ward" : "school";
  if (baseLevel === "state") {
    if (filters.zone) r = r.filter(row => geoMatches(row.zone, filters.zone));
    if (filters.state) { r = r.filter(row => geoMatches(row.state, filters.state)); level = next(level); }
  }
  if (filters.lga && level !== "school") { r = r.filter(row => geoMatches(row.lga, filters.lga)); level = next(level); }
  if (filters.ward && level !== "school") { r = r.filter(row => geoMatches(row.ward, filters.ward)); level = next(level); }
  if (drill.state) { r = r.filter(row => geoMatches(row.state, drill.state)); level = "lga"; }
  if (drill.lga) { r = r.filter(row => geoMatches(row.lga, drill.lga)); level = "ward"; }
  return { level, rows: r };
}

function buildLossRows(metrics: AggregateMetrics): LossRow[] {
  return [
...
  ].map(row => ({ ...row, lost: Math.max(0, row.from - row.to), lostPct: row.from > 0 ? ((row.from - row.to) / row.from) * 100 : 0 }));
}

// ─── Policy layout helper ────────────────────────────────────────────────────
function baseLayout(height = 300): Partial<PlotlyLayout> {
  return {
    height,
    paper_bgcolor: "white",
    plot_bgcolor: "white",
...
  return (
    <div className="min-h-[245px] overflow-x-auto">
...
// ─── UI components (from AccessCoverageDashboardPage) ────────────────────────
...
          <span
            className={item.shape === "circle" ? "inline-block h-2.5 w-2.5 rounded-full" : "inline-block h-2.5 w-4 rounded-sm"}
            style={{
              backgroundColor: item.dashed ? "transparent" : item.color,
              borderTop: item.dashed ? `2px dashed ${item.color}` : undefined,
              border: item.shape === "circle" ? `2px solid ${item.color}` : undefined,
            }}
          />
          {item.label}
        </div>
...
function MetricCardView({ item, prevSessionLabel }: { item: MetricCard; prevSessionLabel?: string }) {
...
              {item.icon}
            </div>
            <div className="text-[12px] font-medium text-slate-500 leading-tight min-w-0">{item.label}</div>
          </div>
          <div
...
        </div>
        {/* Row 2: big number */}
        <div className="mt-2.5 text-[24px] font-bold leading-none tracking-tight text-slate-900 tabular-nums">
          {fmtInt(item.value)}
        </div>
        {/* Row 3: delta + session comparison */}
...
            </div>
            {prevSessionLabel ? (
              <span className="text-[10px] text-slate-400">vs {prevSessionLabel}</span>
            ) : null}
          </div>
        ) : null}
      </div>
...
      <div className="p-3">
        {preBodyContent ? <div className="mb-3">{preBodyContent}</div> : null}
...
// ─── Nigeria SVG map (from AccessCoverageDashboardPage) ─────────────────────
...
            <span className="text-slate-800">{activeState}</span>
          </div>
        )}
...
                  const text = formatTooltip ? formatTooltip(name, v) : `${name}: ${fmtVal(v)}`;
...
        {/* Tooltip */}
        {tip && containerRef.current && (() => {
          const parts = tip.text.split(" — ");
          return (
            <div
              className="pointer-events-none absolute z-50 rounded-xl border border-slate-600 shadow-2xl"
              style={{ right: -136, bottom: 8, width: 212, background: "#0f172a" }}
            >
              <div className="px-3 pt-2.5 pb-2.5">
                <div className="text-[13px] font-bold text-white leading-tight">{parts[0]}</div>
                {parts.slice(1).map((line, i) => (
                  <div key={i} className="mt-1 text-[11px] text-slate-300 leading-snug">{line}</div>
                ))}
              </div>
            </div>
          );
...
function MapChartCard({ title, explanation, mapData, drill, onReset, onStateClick }: MapChartCardProps) {
  const [showHelp, setShowHelp] = useState(false);
...
      {/* ── Expanded map modal ────────────────────────────────── */}
      {expanded && (
        <div className="fixed inset-0 z-[95] flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm" onClick={() => setExpanded(false)}>
          <div ref={expandRef} className="w-full max-w-[820px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"
...
export const GENERAL_OVERVIEW_SECTIONS = [
...
  { id: "general-tertiary", label: "Tertiary & Non-Formal Trends" },
  { id: "general-loans", label: "Student Loan Support" },
] as const;


// ─── Main component ───────────────────────────────────────────────────────────
export default function GeneralOverviewDashboard({
  filters, setFilters, dimSessions, disabilityMode,
}: {
  filters: MinisterFilters;
...
  const [policyRows, setPolicyRows] = useState<PolicyImpactRow[]>([]);
...
    if (disabilityMode && r.disability !== "Disabled") return false;
    return true;
...
    if (disabilityMode && r.disability !== "Disabled") return false;
    return true;
  }), [wardRows, filters.session, filters.zone, disabilityMode]);
...
      return true;
    });
  }, [wardRows, previousSession, filters, disabilityMode]);
...
  const [iqsDrill, setIqsDrill] = useState<DrillState>({});
  const [expandState, setExpandState] = useState<ExpandState>(null);
  const expandRef = useOutsideClose<HTMLDivElement>(!!expandState, () => setExpandState(null));

  // Master reset — clears all drills and location filters
  const resetAll = () => {
setDropoffDrill({}); setIqsDrill({});
...
        help: CARD_HELP.totalStudentsPrimary,
      },
      {
...
        help: CARD_HELP.totalFormalSecondary,
      },
      {
...
  // ── Section 2: Access & Coverage map + drill chart ──────────────────────────
  const densityCombinedMapData = useMemo<SvgMapProps | null>(() => {
    ...
    const baseRows = activeState
      ...
      : densityBaseRows.filter((row) => row.school_level === "Pre-Primary/Primary");

    const groups = aggregateBy(baseRows, level);
    const values: Record<string, number> = {};
    groups.forEach((group) => {
      const m = group.metrics;
      const value = m.schools > 0 ? m.students / m.schools : 0;
    ...
    return {
      level,
      values,
      metricLabel: "Average learners per school",
      colorStart: COLORS.tealStart,
      colorEnd: COLORS.tealEnd,
      formatTooltip: (name: string, val: number) => {
        const scoped = baseRows.filter((row) => (level === "state" ? geoMatches(row.state, name) : geoMatches(row.lga, name)));
  ...
  const densityCombinedDrillChart = useMemo<ChartBundle | null>(() => {
    ...
    const groups = aggregateBy(scopedRows, "lga")
      .map((group) => {
        const publicRows = scopedRows.filter((row) => geoMatches(row.lga, group.label) && row.school_type === "Public");
        const privateRows = scopedRows.filter((row) => geoMatches(row.lga, group.label) && row.school_type === "Private");
        const publicStudents = publicRows.reduce((sum, row) => sum + safeNum(row.student_count), 0);
        const publicSchools = uniqueSchoolFacilityCount(publicRows);
        const privateStudents = privateRows.reduce((sum, row) => sum + safeNum(row.student_count), 0);
        const privateSchools = uniqueSchoolFacilityCount(privateRows);
    ...
    return {
      ...
      fixedLegend: [
        { label: "Public", color: "#0891b2" },
        { label: "Private", color: "#8b5cf6" },
      ],
      expandedWidthClass: "max-w-[920px]",
    };
...
    const values = [metrics.ss3_total, metrics.o_level_candidates, metrics.utme_participants, metrics.admitted_students, metrics.matriculated_students];
    const max = Math.max(...values, 1);
    return {
      data: [{
        type: "funnel", y: labels,
        x: values.map(v => (v / max) * 100),
        text: values.map((v, i) => `${labels[i]}<br><b>${fmtInt(v)}</b>`),
        textposition: "inside", textinfo: "text",
        customdata: values.map((v, i) => [labels[i], v, i === 0 ? "" : `${metrics.o_level_candidates > 0 ? Math.round((v / metrics.o_level_candidates) * 100) : 0}% of O-Level`]),
...
  const lossRows = useMemo(() => buildLossRows(currentMetrics), [currentMetrics]);
...
    const stageDefs = [
      { name: "SS3 → O-Level", color: COLORS.ss3, value: (r: GroupedTrRow) => Math.max(0, r.metrics.ss3_total - r.metrics.o_level_candidates), base: (r: GroupedTrRow) => r.metrics.o_level_candidates },
      { name: "O-Level → UTME", color: COLORS.utme, value: (r: GroupedTrRow) => Math.max(0, r.metrics.o_level_candidates - r.metrics.utme_participants), base: (r: GroupedTrRow) => r.metrics.o_level_candidates },
      { name: "UTME → Admitted", color: COLORS.admit, value: (r: GroupedTrRow) => Math.max(0, r.metrics.utme_participants - r.metrics.admitted_students), base: (r: GroupedTrRow) => r.metrics.o_level_candidates },
      { name: "Admitted → Matric", color: COLORS.matric, value: (r: GroupedTrRow) => Math.max(0, r.metrics.admitted_students - r.metrics.matriculated_students), base: (r: GroupedTrRow) => r.metrics.o_level_candidates },
    ];
    const data: PlotlyData[] = stageDefs.map((stage) => ({
      type: "bar",
      orientation: "h",
      name: stage.name,
      y: labels,
      x: grouped.map((r) => stage.value(r)),
      customdata: grouped.map((r) => {
        const drop = stage.value(r);
        const base = stage.base(r);
        const pct = base > 0 ? Math.round((drop / base) * 100) : 0;
        return [r.label, drop, pct];
      }),
      marker: { color: stage.color },
      text: grouped.map((r) => fmtInt(stage.value(r))),
      texttemplate: "%{text}",
      textposition: "inside",
      insidetextanchor: "middle",
      textfont: { color: "#ffffff", size: 11 },
      constraintext: "none",
      cliponaxis: false,
      hovertemplate: "%{customdata[0]}<br>Drop-off: %{customdata[1]:,.0f}<br>%{customdata[2]}% of O-Level<extra></extra>",
    } as PlotlyData));
    return {
      level: resolved.level,
      bundle: {
        data,
        layout: { ...buildCommonLayout(height), barmode: "stack", showlegend: false, margin: { l: 112, r: 16, t: 6, b: 42 }, yaxis: { showgrid: false, automargin: true, autorange: "reversed" } },
        scrollable: true, scrollMaxHeight: 300, expandedMaxHeight: 360,
...
    ly.showlegend = false;
    (ly as Record<string, unknown>).yaxis = { gridcolor: COLORS.grid, zeroline: false, tickfont: { color: COLORS.sub }, title: { text: "Matriculated learners" } };
    return {
      data: [stemmTrace, nonStemmTrace] as unknown as PlotlyData[],
      layout: ly,
...
    ly.showlegend = false;
    ly.margin = { l: 48, r: 20, t: 36, b: 56 };
    (ly as Record<string, unknown>).yaxis = { gridcolor: COLORS.grid, zeroline: false, tickfont: { color: COLORS.sub }, title: { text: "Volume" } };
    return {
      data: [barTrace, lineTrace] as unknown as PlotlyData[],
      layout: ly,
...
    };
    return {
      data: [iqsBarTrace, iqsLineTrace],
      layout: {
        ...buildCommonLayout(250),
        margin: { l: 58, r: 56, t: 12, b: 102 },
        xaxis: { tickangle: -35, tickfont: { color: COLORS.sub }, categoryorder: "array", categoryarray: labels },
        yaxis: { title: { text: "Enrollment" }, tickfont: { color: COLORS.sub }, gridcolor: COLORS.grid },
        yaxis2: { title: { text: "Schools" }, overlaying: "y", side: "right", tickfont: { color: COLORS.sub }, showgrid: false },
        legend: { orientation: "h", x: 0, y: -0.3, font: { size: 11, color: COLORS.sub } },
      },
...
    <div className="space-y-5">
      ...
      {/* ── Section 1: KPI Cards ── */}
      <section id="general-kpi" className="scroll-mt-36 space-y-4">
        <div className="grid gap-3 md:grid-cols-5">
          {cardMetrics.map(item => <MetricCardView key={item.label} item={item} prevSessionLabel={previousSession || undefined} />)}
        </div>
      </section>

      {/* ── Section 2: Access & School Coverage ── */}
      <section id="general-access" className="scroll-mt-36 space-y-4">
        <div className="grid gap-3 lg:grid-cols-2">
          {densityCombinedDrillChart ? (
            <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-4 py-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setFilters((previous) => ({ ...previous, state: "", lga: "", ward: "", school: "" }))}
            ...
            <MapChartCard
              ...
              explanation="Average primary learner load by state. Hover shows the overall average together with the public and private learner and school totals. Click a state to switch into the LGA breakdown."
              mapData={densityCombinedMapData}
              drill={{ state: filters.state || undefined }}
              onReset={resetAll}
              onStateClick={(name) => setFilters((previous) => ({ ...previous, state: name, lga: "", ward: "", school: "" }))}
            />
          )}
          <ChartCard
            title="Primary Level Pupil-Teacher Ratio by School Type"
            explanation={`Primary pupil-teacher ratio split into public and private bars by location. The benchmark is shown in the legend only.`}
            bundle={ptrChartResult?.bundle}
            onRefresh={resetAll}
            onPlotClick={(event) => {
              const point = event.points?.[0];
              if (!point) return;
              const raw = Array.isArray(point.customdata) ? point.customdata[0] : point.customdata;
              const name = typeof raw === "string" ? raw : (typeof point.y === "string" ? point.y : "");
              if (name && ptrChartResult?.level === "state") {
                setFilters((previous) => ({ ...previous, state: name, lga: "", ward: "", school: "" }));
              } else if (name && ptrChartResult?.level === "lga") {
                setFilters((previous) => ({ ...previous, lga: name, ward: "", school: "" }));
              }
            }}
            onExpand={() =>
              setExpandState({
                key: "ptr",
                title: "Primary Level Pupil-Teacher Ratio by School Type",
                help: `UBE primary benchmark: ${UBEC_BENCHMARK.value}:1. Hover a bar to see the public school type and private school type ratios, and click to drill deeper by location.`,
              })
            }
          />
        </div>
...
      {/* ── Section 3: Transition & Learner Flow ── */}
      <section id="general-transition" className="scroll-mt-36 space-y-4">
        <div className="grid gap-3 lg:grid-cols-2">
          <ChartCard
            title="Direct Transition Funnel"
            explanation="Stage-by-stage funnel from SS3 through to final matriculation. Shows how many learners remain at each step for the selected session and filters."
            bundle={progressionChartBundle}
onExpand={() => setExpandState({ key: "funnel", title: "Direct Transition Funnel", help: "SS3 → O-Level → UTME → Admitted → Matriculated funnel for current filters." })}
          />
          <ChartCard
            title="Student Loss by Stage"
            explanation="Number of learners lost at each transition stage and the percentage of the starting pool that represents."
onExpand={() => setExpandState({ key: "lossTable", title: "Student Loss by Stage", help: "Learner count and rate lost at each pipeline stage." })}
          >
...
          explanation="Stacked horizontal bars showing learners lost at each pipeline stage by state. Click a bar to drill into that state or LGA. Use Reset All to clear."
          bundle={dropoffStateChart.bundle}
          onRefresh={resetAll}
          onPlotClick={(event) => {
            const pt = event.points?.[0];
            if (!pt) return;
            const raw = Array.isArray(pt.customdata) ? pt.customdata[0] : pt.customdata;
            const name = typeof raw === "string" ? raw : (typeof pt.y === "string" ? pt.y : "");
            if (!name) return;
            if (dropoffStateChart.level === "state") {
              setDropoffDrill({ state: name });
              setFilters(p => ({ ...p, state: name, lga: "", ward: "", school: "" }));
            } else if (dropoffStateChart.level === "lga") {
              setDropoffDrill(p => ({ ...p, lga: name }));
              setFilters(p => ({ ...p, lga: name, ward: "", school: "" }));
            }
          }}
onExpand={() => setExpandState({ key: "dropoff", title: "Drop-off by State", help: "Learners lost at each transition stage by state. Click a bar to drill." })}
        />
      </section>

      {/* ── Section 4: Tertiary + Loans (same row) ── */}
      <section id="general-tertiary" className="scroll-mt-36 space-y-4">
        <div className="grid gap-3 lg:grid-cols-2">
          <ChartCard
            title="STEMM & Non-STEMM Matriculated Trend"
            explanation="Matriculated learner counts for STEMM and Non-STEMM programmes across all academic sessions. Only matriculated counts are shown — admitted is excluded — to reflect confirmed enrolment outcomes. Use this to see whether STEMM uptake is growing relative to Non-STEMM over time."
            bundle={stemmNonStemmTrendBundle}
...
        {/* IQS/IQTS — full width below */}
        <ChartCard
          title="Adult & Non-Formal Education (IQS/IQTE) — Schools & Enrollment by State"
          explanation="Bars show Adult & Non-Formal (IQS/IQTE) student enrollment by state. The line shows the number of Adult & Non-Formal centres. Selecting a state in the filter bar or clicking a state bar moves the chart into LGA-level detail. Use Reset All to return to national view."
          bundle={iqsChartBundle}
          onRefresh={resetAll}
          onPlotClick={(event) => {
            const pt = event.points?.[0];
            if (!pt) return;
            const raw = Array.isArray(pt.customdata) ? pt.customdata[0] : pt.customdata;
            const name = typeof raw === "string" ? raw : (typeof pt.x === "string" ? pt.x : "");
            if (!name) return;
            if (filters.state) {
              setFilters((previous) => ({ ...previous, lga: name, ward: "", school: "" }));
            } else {
...
            }
          }}
onExpand={() => setExpandState({ key: "iqs", title: "Adult & Non-Formal Education (IQS/IQTE) — Schools & Enrollment by State", help: "Adult & Non-Formal (IQS/IQTE) enrollment and school count by state or selected-state LGA view." })}
        />
      </section>

      {/* ── Expand Modal — live bundle lookup so drill updates show in real time ── */}
      {expandState ? (() => {
        // Look up the current live bundle/data by key — never stale
        type ExpandEntry = { bundle?: ChartBundle; tableRows?: LossRow[]; onPlotClick?: (e: PlotPointEvent) => void };
        const expandMap: Record<ExpandKey, ExpandEntry> = {
          density: {
            bundle: densityCombinedDrillChart ?? { data: [], layout: buildCommonLayout(10) },
          },
          ptr: {
            bundle: ptrChartResult?.bundle,
            onPlotClick: (e) => {
...
              const raw = Array.isArray(pt.customdata) ? pt.customdata[0] : pt.customdata;
              const name = typeof raw === "string" ? raw : (typeof pt.y === "string" ? pt.y : "");
              if (name && ptrChartResult?.level === "state") setFilters(p => ({ ...p, state: name, lga: "", ward: "", school: "" }));
              else if (name && ptrChartResult?.level === "lga") setFilters(p => ({ ...p, lga: name, ward: "", school: "" }));
            },
          },
          dropoff: {
            bundle: dropoffStateChart.bundle,
...
              const name = typeof raw === "string" ? raw : (typeof pt.y === "string" ? pt.y : "");
              if (!name) return;
              if (dropoffStateChart.level === "state") { setDropoffDrill({ state: name }); setFilters(p => ({ ...p, state: name, lga: "", ward: "", school: "" })); }
              else if (dropoffStateChart.level === "lga") { setDropoffDrill(prev => ({ ...prev, lga: name })); setFilters(p => ({ ...p, lga: name, ward: "", school: "" })); }
            },
          },
          funnel: { bundle: progressionChartBundle },
          lossTable: { tableRows: lossRows },
          stemm: { bundle: stemmNonStemmTrendBundle },
          loan: { bundle: loanTrendBundle },
          iqs: {
            bundle: iqsChartBundle,
            onPlotClick: (e) => {
              const pt = e.points?.[0];
              if (!pt) return;
              const raw = typeof pt.customdata === "string" ? pt.customdata : (Array.isArray(pt.customdata) ? String(pt.customdata[0]) : "");
              const name = raw || (typeof pt.x === "string" ? pt.x : "");
              if (!name) return;
              if (filters.state) { setFilters((p) => ({ ...p, lga: name, ward: "", school: "" })); }
              else { setFilters((p) => ({ ...p, state: name, lga: "", ward: "", school: "" })); }
            },
          },
        };
        const entry = expandMap[expandState.key];
...
                bundle?.expandedWidthClass ?? "max-w-[1080px]"].join(" ")}
              onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between gap-4 border-b border-slate-100 px-5 py-4">
                <div>
                  <div className="text-base font-bold text-slate-900">{expandState.title}</div>
                  <div className="mt-0.5 text-xs text-slate-400">{expandState.help}</div>
                </div>
                <button type="button" onClick={() => setExpandState(null)}
                  className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50">
                  <X className="h-4 w-4" />
                </button>
              </div>
...
                    {bundle.fixedLegend?.length ? <FixedLegend items={bundle.fixedLegend} /> : null}
                    <div
                      className={bundle.scrollable ? "overflow-y-auto pr-1" : undefined}
                      style={bundle.scrollable ? { maxHeight: bundle.expandedMaxHeight ?? 420 } : undefined}
                    >
                      <Plot
                        data={bundle.data}
...
                        config={bundle.config ?? { displayModeBar: false, responsive: true }}
                        style={{ width: "100%", height: "100%" }}
                        onClick={entry.onPlotClick}
                      />
                    </div>
                  </>
                ) : null}
                {entry.tableRows ? <SummaryTable rows={entry.tableRows} /> : null}
              </div>
            </div>
          </div>
        );
